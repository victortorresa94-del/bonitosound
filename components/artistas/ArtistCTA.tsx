import Link from "next/link";
import { Section, Cta } from "@/components/ui";
import { RevealOnScroll, MagneticButton } from "@/components/motion";
import { serverLocale } from "@/lib/locale-server";
import { tr } from "@/lib/copy-ca";

/**
 * Cierre de la ficha de artista: booking + seguir + volver al roster.
 * Plug-and-play: sin nombre o sin email de booking no renderiza nada.
 */
export function ArtistCTA({
  name,
  bookingEmail,
  instagram,
  slug,
  pasado,
}: {
  name: string;
  bookingEmail: string;
  instagram?: string;
  slug: string;
  /** La colaboración ya terminó: no se puede contratar a quien no llevamos. */
  pasado?: boolean;
}) {
  const locale = serverLocale();
  // Datos esenciales: sin ellos el bloque no tiene sentido.
  const nombre = name?.trim();
  const email = bookingEmail?.trim();
  if (!nombre || !email) return null;

  // El botón lleva al formulario único de contratación ya precargado con este
  // artista (?a=<slug>), no a un mailto pelado: se pide la info que hace falta.
  const contratarHref = `/contacto?a=${slug}`;

  // Colaboración terminada: el cierre no es un formulario de contratación sino
  // una puerta al roster. Ni prometemos lo que no podemos dar ni dejamos la
  // ficha sin salida.
  if (pasado) {
    return (
      <Section id={slug ? `mas-${slug}` : "mas"}>
        <div className="rounded-3xl border border-subtle bg-bg-secondary px-6 py-14 text-center sm:px-8 md:py-20">
          <RevealOnScroll as="p" className="eyebrow">
            Trabajamos juntos
          </RevealOnScroll>
          <RevealOnScroll as="h2" delay={0.05}
            className="display mx-auto mt-4 max-w-3xl text-[clamp(2rem,4.5vw,3.4rem)] leading-[1.05] text-text-primary">
            ¿Y a quién llevamos <span className="text-[#16b6d4]">ahora</span>?
          </RevealOnScroll>
          <RevealOnScroll delay={0.18} className="mt-10 flex flex-wrap items-center justify-center gap-4">
            <MagneticButton strength={0.3}>
              <Cta href="/artistas" variant="primary">Saber más →</Cta>
            </MagneticButton>
          </RevealOnScroll>
          <RevealOnScroll delay={0.24} className="mt-8">
            <Link href="/artistas/todos"
              className="text-sm font-semibold text-text-muted underline-offset-4 transition-colors hover:text-text-primary hover:underline">
              Ver todo el roster →
            </Link>
          </RevealOnScroll>
        </div>
      </Section>
    );
  }

  return (
    <Section id={slug ? `booking-${slug}` : "booking"}>
      <div className="rounded-3xl border border-subtle bg-bg-secondary px-6 py-14 text-center sm:px-8 md:py-20">
        <RevealOnScroll as="p" className="eyebrow">
          {tr(locale, "Booking")}
        </RevealOnScroll>

        <RevealOnScroll
          as="h2"
          delay={0.05}
          className="display mx-auto mt-4 max-w-3xl text-[clamp(2rem,4.5vw,3.4rem)] leading-[1.05] text-text-primary"
        >
          Hablemos de <span className="text-[#16b6d4]">{nombre}</span>.
        </RevealOnScroll>

        <RevealOnScroll
          as="p"
          delay={0.12}
          className="mx-auto mt-5 max-w-xl text-base leading-relaxed text-text-secondary md:text-lg"
        >
          {tr(locale, "Cuéntanos fecha, sitio y qué tienes en mente. Te contamos disponibilidad")}{" "}
          {tr(locale, "y cómo montarlo, sin vueltas.")}
        </RevealOnScroll>

        <RevealOnScroll
          delay={0.18}
          className="mt-10 flex flex-wrap items-center justify-center gap-4"
        >
          <MagneticButton strength={0.3}>
            <Cta href={contratarHref} variant="primary">
              Contratar a {nombre} →
            </Cta>
          </MagneticButton>
        </RevealOnScroll>

        <RevealOnScroll delay={0.24} className="mt-8">
          <Link
            href="/artistas/todos"
            className="text-sm font-semibold text-text-muted underline-offset-4 transition-colors hover:text-text-primary hover:underline"
          >
            {tr(locale, "Ver todo el roster →")}
          </Link>
        </RevealOnScroll>
      </div>
    </Section>
  );
}
